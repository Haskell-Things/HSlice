...
data Point a = Point { x :: a, y :: a, z :: a }
...
-- TODO (1): Does it make sense to have separate types for lines and line segments? Will we
-- ever need an infinite line?
-- TODO (2): Is this the best representation, or does it make sense to just have a line
...
-- Find the point on a line for a given Z value. Note that this evaluates to 0
...
-- lines
readFacet :: (Num a, Read a) => [String] -> [Line a]
readFacet f
    | length f < 3 = error "Invalid facet"
    | otherwise = makeLines $ map readPoint f'
...
facetLinesFromSTL :: (Num a, Read a) => [String] -> [[Line a]]
