...
prop_ScalarDotScalar v1 v2 v3 v4 = scalarPart (rawPPoint2 (v1,v2) ⋅ rawPPoint2 (v3,v4)) == (-1)
  where
    rawPPoint2 (x,y) = (\(CPPoint2 v) -> v) $ eToPPoint2 (Point2 (x,y))
...
prop_LineSegIntersectionStableAtOrigin d1 x1 y1 rawX2 rawY2
  ...
  | otherwise = error
                ...
                <> "(x2,y2): " <> show (x2,y2) <> "\n"
  where
    res1 = intersectsWithErr (Right pLineThroughOriginNotX1Y1NotOther) (Left (x1y1LineSegToOrigin,mempty))
    res2 = intersectsWithErr (Right pLineThroughOriginNotX1Y1NotOther) (Left (lineSegFromOrigin,mempty))
...
prop_LineSegIntersectionStableAtX1Y1Point pointD rawD1 x1 y1 rawX2 rawY2
  ...
  where
    res1 = intersectsWithErr (Right pLineThroughPointNotX1Y1NotOther) (Left (x1y1LineSegToPoint,mempty))
    res2 = intersectsWithErr (Right pLineThroughPointNotX1Y1NotOther) (Left (lineSegFromPointNotX1Y1,mempty))
    distanceStart = case res2 of
...
prop_PPointWithinErrRange x y
  ...
  where
    (res, UlpSum resErr) = distance2PP p1 p2
    p1 = makePPoint2 x y
    p2 = makePPoint2 x y
...
prop_PLineIntersectsAtXAxis x y rawX2 y2 m
  ...
  where
    ...
    (foundDistance, distanceErr) = distance2PP axisIntersectionPoint intersectionPPoint2
