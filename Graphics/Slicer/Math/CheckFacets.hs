...
data FacetWithNeighbors = FacetWithNeighbors { self :: Facet, neighbor1 :: Facet, neighbor2 :: Facet, neighbor3 :: Facet}
  deriving (Eq)
...
findNeighbors facet@(Facet (s1,s2,s3) _) facets = FacetWithNeighbors facet (findOtherFacetWithSide s1 facet facets) (findOtherFacetWithSide s2 facet facets) (findOtherFacetWithSide s3 facet facets)
  where
    ...
    findOtherFacetWithSide side target fs = if length res == 2 then head $ filter (\a -> a /= target) res else error $ show res
      where
