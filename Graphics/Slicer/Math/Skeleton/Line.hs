...
addLineSegsToFace distance insets face@(Face edge firstArc midArcs@(Slist rawMidArcs _) lastArc)
  ...
  where
    ...
    translateDir v         = case pLineIsLeft (eToPLine2WithErr edge) (firstArc, mempty) of
                               (Just True) -> (-v)
                               (Just False) -> v
    ...
    foundLineSegs          = [ makeLineSeg (pToEPoint2 $ intersectionOf newSide (firstArc,mempty)) (pToEPoint2 $ intersectionOf newSide (lastArc,mempty)) | newSide <- newSides ]
      where
        newSides = [ translatePLine2WithErr (eToPLine2 edge) $ translateDir (-(distance+(distance * fromIntegral segmentNum))) | segmentNum <- [0..linesToRender-1] ]
    ...
    finalSide              = makeLineSeg (pToEPoint2 $ intersectionOf (finalLine,mempty) (firstArc,mempty)) (pToEPoint2 $ intersectionOf (finalLine,mempty) (lastArc,mempty))
      where
    ...
    distanceUntilEnd = case midArcs of
                         (Slist [] 0) -> distancePPointToPLine (intersectionOf (firstArc,mempty) (lastArc,mempty),mempty) (eToPLine2WithErr edge)
                         (Slist [oneArc] 1) -> if firstArcLonger
                                               then distancePPointToPLine (intersectionOf (firstArc,mempty) (oneArc,mempty) ,mempty) (eToPLine2WithErr edge)
                                               else distancePPointToPLine (intersectionOf (oneArc,mempty) (lastArc,mempty) ,mempty) (eToPLine2WithErr edge)
    ...
    threeSideRemainder     = if distancePPointToPLine (intersectionOf (firstArc,mempty) (midArc,mempty),mempty) (eToPLine2WithErr edge) /= distancePPointToPLine (intersectionOf (midArc,mempty) (lastArc,mempty),mempty) (eToPLine2WithErr edge)
                             then subRemains
                             else Nothing
    ...
    firstArcLonger         = distancePPointToPLine (intersectionOf (firstArc,mempty) (midArc,mempty) ,mempty) (eToPLine2WithErr edge) > distancePPointToPLine (intersectionOf (midArc,mempty) (lastArc,mempty),mempty) (eToPLine2WithErr edge)
    ----------------------------------------------
