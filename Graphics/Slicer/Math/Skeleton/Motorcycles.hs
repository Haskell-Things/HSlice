...
import Graphics.Slicer.Math.PGA (CPPoint2, NPLine2(NPLine2), PLine2(PLine2), PLine2Err(PLine2Err), PPoint2, Arcable(outOf), Pointable(canPoint, ePointOf, pPointOf), eToPL, flipL, normalizeL, pLineIsLeft, pPointsOnSameSideOfPLine, PIntersection(IntersectsIn,PAntiCollinear), plinesIntersectIn, angleBetween2PL, outputIntersectsLineSeg)
    ...
    filterIntersection intersection = case intersection of
      ...
      where
        intersectionPointIsBehind point = angleFound < realToFrac (ulpVal angleErr)
          where
            (angleFound, (_,_, angleErr)) = angleBetween2PL (normalizeL $ outOf motorcycle) (eToPL $ lineSegToIntersection point)
        intersectionCPPointIsBehind pPoint = angleFound < realToFrac (ulpVal angleErr)
          where
            (angleFound, (_,_, angleErr)) = angleBetween2PL (normalizeL $ outOf motorcycle) (eToPL $ lineSegToIntersectionP pPoint)
