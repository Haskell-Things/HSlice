...
randomStarPoly centerX centerY radianDistPairs = fromMaybe dumpError $ maybeFlipContour contour
  where
    ...
    pointsAroundCenter = (\(distanceFromPoint, angle) -> translateRotatePPoint2WithErr centerPPoint (coerce distanceFromPoint) (coerce angle)) <$> radianDistPairs
...
randomENode x y d1 rawR1 d2 rawR2 = makeENode p1 intersectionPoint p2
  where
    ...
    pp1 = translateRotatePPoint2WithErr intersectionPPoint (coerce d1) (coerce r1)
    pp2 = translateRotatePPoint2WithErr intersectionPPoint (coerce d2) (coerce r2)
    p1 = pToEPoint2 pp1
    p2 = pToEPoint2 pp2
