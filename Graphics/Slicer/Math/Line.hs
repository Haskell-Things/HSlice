...
module Graphics.Slicer.Math.Line (Line(Line), point, slope, lineFromEndpoints, rawLineFromEndpoints, pointsFromLines, endpoint, midpoint, flipLine, pointSlopeLength, perpendicularBisector, pointAtZValue, shortenLineBy, makeLines, makeLinesLooped, lineSlope, Direction(Positive, Negative), Slope(IsOrigin, OnXAxis, OnYAxis, HasSlope), Intersection (IntersectsAt, NoIntersection, Parallel, HitEndpointL1, HitEndpointL2, Collinear), angleOf, SearchDirection(Clockwise,CounterClockwise), lineBetween, combineLines) where
