...
module Graphics.Slicer.Math.Arcs (getFirstArc, getOutsideArc, towardIntersection, getInsideArc) where
...
getOutsideArc ppoint1 pline1 ppoint2 pline2
  ...
  | fst intersectionPoint == cppoint2 = error $ "intersection of plines is at second ppoint:\n"
                                          ...
                                          <> show pline1 <> "\n"
                                          <> show pline2 <> "\n"
  | l1TowardPoint && l2TowardPoint = flipFst $ getInsideArc npline1 (flipL npline2)
  | l1TowardPoint                  = flipFst $ getInsideArc npline1 npline2
  | l2TowardPoint                  = getInsideArc npline1 npline2
  | otherwise                      = getInsideArc npline1 (flipL npline2)
    where
